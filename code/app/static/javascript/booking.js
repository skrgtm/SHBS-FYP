    ...
    success: (result) => {
      if (result.status === "success") {
        ...
        $(".amount").each(function () {
          const amountText = $(this).text().replace("Rs", "").replace("Rs", "");
        ...
        if ($("#original_amount").length) {
          ...
          $("#discount").html(`Rs ${newDiscount}`);
          $("#total_amount").html(`Rs ${totalAmount - newDiscount}`);
          $("#original_amount").html(`Rs ${totalAmount}`);
          const payNowLink = document.getElementById("pay_now_link");
          const newHref = `/order_products?total_amount=${
