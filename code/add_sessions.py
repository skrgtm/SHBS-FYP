...
def create_activities():
    ...
    swimming_activities = ["General use",
                           "Lane swimming", "Lessons", "Team events"]
...
def create_sessions():
    ...
    for day in swimming_days:
        create_session(swimming_pool, swimming_start_time,
                       swimming_end_time, ["Team events"], day)
    ...
    for day in range(7):  # Every day of the week
        for start_time, end_time in swimming_pool_hours:
            if not ((day == 4 and start_time >= time(8, 0) and start_time < time(10, 0)) or
                ...
                create_session(swimming_pool, start_time,
                               end_time, swimming_activities, day)
    ...
    for day in range(7):  # Every day of the week
        for start_time, end_time in fitness_room_hours:
            create_session(fitness_room, start_time,
                           end_time, fitness_activity, day)

    # Climbing Wall: General Use
    climbing_activity = ["General use"]
    climbing_room_hours = [(time(hour), time(hour + 1))
                           for hour in range(8, 22)]
    for day in range(7):  # Every day of the week
    ...
    for day in range(7):  # Every day of the week
        for start_time, end_time in fitness_room_hours:
            create_session(squash_court1, start_time,
                           end_time, fitness_activity, day)
            create_session(squash_court2, start_time,
                           end_time, fitness_activity, day)
    ...
    for day in range(7):  # Every day of the week
        for start_time, end_time in sports_hall_hours:
            if not ((day == 3 and start_time >= time(19, 0) and start_time < time(21, 0)) or
                ...
                create_session(sports_hall, start_time,
                               end_time, sports_hall_activity, day)
...
def create_session(facility, start_time, end_time, activity_names, day=None, weeks=2):
    for week in range(weeks):
        if day is not None:
            ...
            next_day = now + \
                timedelta(days=((day - now.weekday()) % 7) + 7 * week)
            date = next_day
        else:
            date = datetime.utcnow().date() + timedelta(days=7 * week)

        session = Sessions(Date=date, Start_time=start_time, End_time=end_time,
                           Remaining_Cap=facility.Capacity, facility_id=facility.id)

        for activity_name in activity_names:
            activity = Activity.query.filter_by(
                Activity_Name=activity_name, facility_id=facility.id).first()
...
def create_facilities():
    ...
    for name, capacity, start_facility, end_facility in facilities_data:
        facility = Facility(Name=name, Capacity=capacity,
                            Start_Facility=start_facility, End_Facility=end_facility)
...
(swimming_pool, fitness_room, squash_court1, squash_court2,
 sports_hall, climbing_wall, studio) = create_facilities()
create_activities()
create_sessions()
